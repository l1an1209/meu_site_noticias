...
class ComercialKiwifyTests(MockResendMixin, TestCase):
    ...
    def test_home_plataforma_e_landing_saas_e_tenant_continua_portal(self):
        ...
        self.assertEqual(resp.status_code, 200)
        html = resp.content.decode()
        self.assertIn('Seu portal. Sua audiência. Sua oportunidade.', html)
        self.assertIn('Criar meu portal', html)
        self.assertIn('Ver planos', html)
        self.assertIn('href="#planos">Criar meu portal', html)
        self.assertIn('href="#planos">Ver planos', html)
        self.assertIn(reverse('pagina_vendas'), html)
        self.assertIn('home-saas', html)
        self.assertIn('Começar agora', html)
        self.assertIn('<h3>Start</h3>', html)
        self.assertIn('<h3>Pro</h3>', html)
        self.assertIn('<h3>Premium</h3>', html)
        self.assertIn('data-pup-event="click_subscribe"', html)
        self.assertIn('data-pup-event="click_plan"', html)
        self.assertIn(reverse('pagina_checkout_plano', args=['basico']), html)
        self.assertIn(reverse('pagina_checkout_plano', args=['profissional']), html)
        self.assertIn(reverse('pagina_checkout_plano', args=['premium']), html)
        self.assertIn('R$ 29,90', html)
        self.assertIn('Pronto para criar seu próprio portal?', html)
        self.assertIn('Comece seu portal hoje', html)
        ...
        self.assertNotContains(tenant, 'home-saas')
